...
test("simplified Jarvis hub is browser-valid and exposes only three primary areas", async () => {
  ...
  assert.match(hub, /id: "home", label: "JARVIS"/);
  assert.match(hub, /id: "brain", label: "Gehirn"/);
  assert.match(hub, /id: "system", label: "System"/);
  assert.doesNotMatch(hub, /id: "integrations", label:/);
  assert.doesNotMatch(hub, /id: "models", label:/);
