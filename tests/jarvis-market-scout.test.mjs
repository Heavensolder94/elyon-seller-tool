...
test("market scout enqueues a browser-independent read-only task", async () => {
  ...
  assert.match(result.task.statusUrl, /^https:\/\/worker\.example\.test\/market-scout\/tasks\//);
  ...
  assert.equal(typeof payload.payload.statusToken, "string");
  assert.ok(payload.payload.statusToken.length > 20);
});
