...
function getUpstashConfig() {
  return {
    url: process.env.GOOGLE_SHEETS_SYNC_STORE_URL || "",
    token: process.env.GOOGLE_SHEETS_SYNC_STORE_TOKEN || "",
  };
}
...
async function readUpstash() {
  ...
  const response = await fetch(`${url.replace(/\/+$/, "")}/get/${encodeURIComponent(getStoreKey())}`, {
    headers: {
...
async function writeUpstash(payload) {
  ...
  const response = await fetch(`${url.replace(/\/+$/, "")}/set/${encodeURIComponent(getStoreKey())}`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${token}`,
