...
const MAX_LIMIT = 100;
const VALID_STATES = new Set(["unread", "opened", "approved", "rejected", "archived", "trashed", "deleted"]);
const RESTORABLE_STATES = new Set(["unread", "opened", "approved", "rejected", "archived"]);
...
function normalizeState(row = {}) {
  ...
  const previousState = RESTORABLE_STATES.has(text(row.previous_state, 40)) ? text(row.previous_state, 40) : null;
  return {
    state,
    previousState,
    ...
    trashedAt: text(row.trashed_at, 100) || null,
    deletedAt: text(row.deleted_at, 100) || null,
...
function candidateItem(task, candidate, index, state = {}) {
  ...
  const workflow = normalizeState(state);
  return {
...
async function listStateRows(taskIds, env = process.env) {
  ...
  const rows = await supabaseJarvisRequest(
    `/rest/v1/jarvis_inbox_state?select=task_id,item_key,source_type,state,previous_state,read_at,approved_at,rejected_at,archived_at,trashed_at,deleted_at,nova_transfer_status,nova_import_id,nova_transferred_at,nova_transfer_error,updated_at&task_id=in.${filter}`,
    { method: "GET" },
    env
  );
...
async function listJarvisInbox({ limit = DEFAULT_LIMIT, env = process.env } = {}) {
  ...
  for (const task of tasks) {
    ...
    if (task.status === "completed" && candidates.length) {
      candidates.forEach((candidate, index) => {
        const itemKey = itemKeyForCandidate(candidate, index);
        const item = candidateItem(task, candidate, index, states.get(stateKey(task.id, itemKey)) || {});
        if (item.state !== "deleted") items.push(item);
      });
      continue;
    }
    if (["failed", "cancelled"].includes(task.status)) {
      const item = failureItem(task, states.get(stateKey(task.id, "task:error")) || {});
      if (item.state !== "deleted") items.push(item);
    }
  }
  ...
  const counts = items.reduce((acc, item) => {
    ...
    return acc;
  }, { unread: 0, opened: 0, done: 0, errors: 0, trash: 0, total: items.length });
...
function statePatch(action, currentState = "unread", previousState = null, now = new Date().toISOString()) {
  ...
  if (action === "trash") {
    const restorable = RESTORABLE_STATES.has(currentState) ? currentState : "unread";
    return { state: "trashed", previous_state: restorable, trashed_at: now, deleted_at: null };
  }
  if (action === "restore") {
    const target = RESTORABLE_STATES.has(previousState) ? previousState : "unread";
    return { state: target, previous_state: null, trashed_at: null, deleted_at: null };
  }
  if (action === "delete_permanent") return { state: "deleted", deleted_at: now, trashed_at: currentState === "trashed" ? now : null };
  throw new Error("jarvis_inbox_invalid_action");
}

async function readCurrentState(taskId, itemKey, env = process.env) {
  const rows = await supabaseJarvisRequest(
    `/rest/v1/jarvis_inbox_state?select=state,previous_state&task_id=eq.${encodeURIComponent(taskId)}&item_key=eq.${encodeURIComponent(itemKey)}&limit=1`,
    { method: "GET" },
    env
  );
  const row = Array.isArray(rows) ? rows[0] : null;
  const normalized = normalizeState(row || {});
  return { state: normalized.state, previousState: normalized.previousState };
}
...
async function updateJarvisInboxState({ taskId, itemKey, action, patch = {}, env = process.env } = {}) {
  ...
  const current = action ? await readCurrentState(cleanTaskId, cleanItemKey, env) : { state: "unread", previousState: null };
  const base = action ? statePatch(action, current.state, current.previousState, now) : {};
