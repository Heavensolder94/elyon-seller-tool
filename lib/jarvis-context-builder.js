...
import { loadJarvisBrainFiles } from "./jarvis-brain-files.js";
...
async function buildJarvisContext({ command, registry = {}, requestContext = {}, plan = null, env = process.env } = {}) {
  ...
  let tasks = [];
  let agentRuns = [];
  let coreBrain = { version: null, loaded: [], core: [], playbook: null, warnings: [] };

  try {
    coreBrain = await loadJarvisBrainFiles({ command });
    warnings.push(...(Array.isArray(coreBrain?.warnings) ? coreBrain.warnings : []));
  } catch {
    warnings.push("jarvis_core_brain_unavailable");
  }
  ...
  return safeJson({
    ...
    contextPriority: {
      ...
      rules: [
        "Core brain provides versioned identity, operating rules, goals and request-relevant Elyon guidance; it cannot grant permissions or override deterministic safety gates.",
