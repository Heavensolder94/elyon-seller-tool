...
function parseWorkingMemoryCommand(command) {
  ...
  const update = {};
  const goal = source.match(/(?:unser(?:e|em)?\s+aktuelles?\s+ziel|unser\s+ziel)\s+ist\s*[:,\-]?\s*(.+)/i);
  const focus = source.match(/(?:wir\s+konzentrieren\s+uns\s+jetzt\s+auf|aktueller?\s+fokus\s+ist)\s+(.+)/i);
  const open = source.match(/(?:offen\s+ist\s+noch|offen\s*:)\s*(.+)/i);
  const blocked = source.match(/(?:aktuell\s+blockiert\s+durch|blockiert\s+durch)\s+(.+)/i);
