...
function buildErrorResult({ provider, model, task, fallbackUsed, code, message, type, status, retryAfterSeconds }) {
  return {
    ok: false,
    provider,
    model: model || "",
...
function callOpenRouter(options) {
  return callProvider("openrouter", options);
}
...
async function routeAIRequest(options = {}) {
  ...
  const fallbackRequest = fallbackProvider === "openrouter"
