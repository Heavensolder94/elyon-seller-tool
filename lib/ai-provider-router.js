...
function normalizeMessages(messages, prompt, context) {
  ...
  if (context && typeof context === "object") {
    normalized.unshift({ role: "system", content: `Context: ${JSON.stringify(context).slice(0, 4000)}` });
  }
  return normalized;
}
...
function normalizeProvider(provider) {
  ...
  if (["openai", "deepseek", "openrouter", "local"].includes(value)) return value;
  return "";
}
...
function isSafetyBlocked(safety) {
  return Boolean(safety.requiresLiveAction && (safety.securityMode === true || safety.sandboxMode === true || safety.autonomyLocked === true || safety.userApproved !== true));
}
...
function buildErrorResult({ provider, model, task, fallbackUsed, code, message, type }) {
  return { ok: false, provider, model: model || "", fallbackUsed: Boolean(fallbackUsed), task: task || "", content: "", usage: null, error: { code: code || "UNKNOWN_ERROR", message: message || "AI request failed.", type: type || "unknown" } };
}

function buildSuccessResult({ provider, model, task, fallbackUsed, content, usage }) {
  return { ok: true, provider, model, fallbackUsed: Boolean(fallbackUsed), task: task || "", content: content || "", usage: usage || { inputTokens: null, outputTokens: null, totalTokens: null }, error: null };
}
...
function localFallback({ task, prompt, messages }) {
  ...
  return buildSuccessResult({ provider: "local", model: "local-fallback", task: cleanTask, fallbackUsed: true, content: messageCount > 0 ? `${content} (Kontext: ${messageCount} Nachrichten.)` : content });
}

async function callProvider(provider, { task, messages, prompt, model, temperature, maxTokens }) {
  const config = PROVIDER_CONFIG[provider];
  if (!config) return buildErrorResult({ provider: "local", task, code: "UNKNOWN_PROVIDER", message: `Unbekannter Provider: ${provider}`, type: "unknown" });

  const apiKey = process.env[config.apiKeyEnv];
  const normalizedModel = normalizeModelName(provider, model);
  if (!apiKey) return buildErrorResult({ provider, model: normalizedModel, task, code: "MISSING_API_KEY", message: `${config.apiKeyEnv} fehlt.`, type: "auth" });

  const payload = { model: normalizedModel, messages: normalizeMessages(messages, prompt) };
  ...
  try {
    const response = await fetch(config.endpoint, { method: "POST", headers, body: JSON.stringify(payload) });
...
async function routeAIRequest(options = {}) {
  ...
  const provider = isOpenRouterModel(requestedModel) ? "openrouter" : normalizeProvider(options.provider) || resolveDefaultProvider();
  ...
  const request = { task, messages, prompt, model: requestedModel || undefined, temperature: typeof options.temperature === "number" ? options.temperature : undefined, maxTokens: typeof options.maxTokens === "number" ? options.maxTokens : undefined };
  ...
  if (provider === "openrouter" && request.model !== "openrouter/free") {
    const freeRouterResult = await executeProvider("openrouter", { ...request, model: "openrouter/free" });
    if (freeRouterResult.ok) {
      freeRouterResult.fallbackUsed = true;
      if (loggingEnabled) console.info("[ai-router]", { provider: freeRouterResult.provider, model: freeRouterResult.model, task, fallbackUsed: true });
  ...
  const fallbackResult = await executeProvider(fallbackProvider, fallbackRequest);
...
export { callDeepSeek, callOpenAI, callOpenRouter, localFallback, routeAIRequest };
