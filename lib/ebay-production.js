...
export function isEbayOfferNotFound(error) {
  const errors = error?.details?.errors;
  return error?.status === 404 && Array.isArray(errors) && errors.length > 0
    && errors.every(entry => Number(entry?.errorId) === 25713);
}

async function resolveDraftOffer(root, headers, built) {
  let data;
  try {
    data = await callEbayJson(`${root}/sell/inventory/v1/offer?sku=${encodeURIComponent(built.listing.sku)}&limit=200`, { headers });
  } catch (error) {
    if (!isEbayOfferNotFound(error)) throw error;
    data = { offers: [], total: 0 };
  }
  const offers = Array.isArray(data.offers) ? data.offers : [];
  if ((!Array.isArray(data.offers) && data.total !== 0) || data.next || Number(data.total) > offers.length) {
    throw serviceError(409, "ebay_offer_lookup_incomplete", "Die vorhandenen Angebote konnten nicht vollständig geprüft werden.");
  }
  // Inventory data is shared by all offers for a SKU, including other markets.
  if (offers.some(offer => text(offer.status).toUpperCase() !== "UNPUBLISHED")) {
    throw serviceError(409, "ebay_draft_existing_offer_not_unpublished", "Für diese SKU besteht ein aktives oder ungeklärtes Angebot. Entwurfsspeichern wurde gestoppt.");
  }
  const candidates = offers.filter(offer => offer.marketplaceId === built.offerPayload.marketplaceId && offer.format === built.offerPayload.format);
  const explicitId = built.listing.offerId;
  if (explicitId) {
    const existing = await callEbayJson(`${root}/sell/inventory/v1/offer/${encodeURIComponent(explicitId)}`, { headers });
    if (existing.sku !== built.listing.sku || existing.marketplaceId !== built.offerPayload.marketplaceId
      || existing.format !== built.offerPayload.format || existing.status !== "UNPUBLISHED") {
      throw serviceError(409, "ebay_draft_offer_mismatch", "Der gewählte Entwurf passt nicht zu SKU, Marktplatz oder Entwurfsstatus.");
    }
    return explicitId;
  }
  if (candidates.length > 1 || candidates.some(offer => !text(offer.offerId))) {
    throw serviceError(409, "ebay_draft_offer_ambiguous", "Der vorhandene eBay-Entwurf ist nicht eindeutig zuordenbar.");
  }
  return text(candidates[0]?.offerId, 64);
}
...
export async function createOrUpdateEbayDraft(input, environment) {
  ...
  const root = ebayApiRoot(session.environment);
  let offerId = await resolveDraftOffer(root, authHeaders(session.accessToken), built);
  ...
  if (verifiedOffer.status !== "UNPUBLISHED" || verifiedOffer.sku !== built.listing.sku
    || verifiedOffer.marketplaceId !== built.offerPayload.marketplaceId) {
    throw serviceError(409, "ebay_draft_verification_failed", "eBay hat den erwarteten unveröffentlichten Entwurf nicht bestätigt.", { offerId, sku: built.listing.sku });
  }
