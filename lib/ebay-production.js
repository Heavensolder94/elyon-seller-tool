...
export async function refreshEbayAccessToken(environment, refreshToken) {
  ...
  const response = await fetch(tokenEndpoint(environment), {
    ...
    body: new URLSearchParams({
      ...
      scope: configuredEbayScopes().join(" "),
    }),
  });
...
function normalizedContact(value, responsible = false) {
  ...
  if (responsible) contact.types = unique(source.types?.length ? source.types : ["EUResponsiblePerson"]).slice(0, 5);
  return Object.fromEntries(Object.entries(contact).filter(([, entry]) => Array.isArray(entry) ? entry.length : Boolean(entry)));
}
...
function extractListing(body) {
  ...
  const manufacturer = normalizedContact(compliance.manufacturer || {
  ...
  const responsiblePerson = normalizedContact(compliance.responsiblePerson || {
