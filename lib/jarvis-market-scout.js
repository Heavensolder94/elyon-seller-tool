...
const DEFAULT_MARKET_SCOUT_TIMEOUT_MS = 90000;
...
async function withTimeout(promise, timeoutMs, timeoutResult) {
  const duration = Math.max(100, Number(timeoutMs) || DEFAULT_MARKET_SCOUT_TIMEOUT_MS);
  let timer = null;
  const timeout = new Promise((resolve) => {
    timer = setTimeout(() => resolve(timeoutResult), duration);
  });
  try {
...
async function runMarketScout({ command, route = routeAIRequest, timeoutMs = DEFAULT_MARKET_SCOUT_TIMEOUT_MS } = {}) {
  ...
  const timeoutResult = {
    ...
    error: {
      code: "TIMEOUT",
      message: `Market Scout research exceeded ${Math.max(100, Number(timeoutMs) || DEFAULT_MARKET_SCOUT_TIMEOUT_MS)} ms.`,
      type: "timeout",
    },
  };
  const result = await withTimeout(route({
    provider: "openrouter",
    model: "openrouter/free",
  ...
  }), timeoutMs, timeoutResult);
  if (!result?.ok) {
    const timedOut = result?.error?.code === "TIMEOUT";
    return {
      ok: false,
      mode: "market_scout_degraded",
      reason: timedOut ? "market_scout_timeout" : "market_scout_provider_unavailable",
      requestedCount: constraints.requestedCount,
      summary: timedOut
        ? "Die Markt-Recherche hat ihr Zeitlimit erreicht, bevor belastbare Produktkandidaten vorlagen. Es wurden keine Kandidaten erfunden."
        : "Für die Markt-Recherche war gerade keine verlässliche Provider-Antwort verfügbar. Es wurden keine Kandidaten erfunden.",
      warnings: [timedOut
        ? "Market Scout wurde kontrolliert beendet, bevor Vercels Request-Limit erreicht wurde."
        : "Keine verlässliche Research-Antwort verfügbar; es wurden keine Kandidaten erfunden."],
      safety: { draftOnly: true, nothingMutated: true },
    };
  }
  const parsed = extractJson(result.content);
  if (!parsed || !Array.isArray(parsed.candidates)) return { ok: false, mode: "market_scout_degraded", reason: "market_scout_invalid_response", requestedCount: constraints.requestedCount, summary: "Die Markt-Recherche lieferte kein verwertbares strukturiertes Ergebnis. Es wurden keine Kandidaten erfunden.", warnings: ["Research-Antwort war nicht im erwarteten strukturierten Format."], safety: { draftOnly: true, nothingMutated: true } };
...
export { DEFAULT_MARKET_SCOUT_TIMEOUT_MS, isMarketScoutCommand, parseConstraints, runMarketScout, WEB_SEARCH_TOOLS };
