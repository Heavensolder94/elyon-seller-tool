...
function brainSystemPrompt() {
  return [
    "You are Elyon Jarvis Brain V2-A.1, the central executive assistant and orchestrator for the Elyon Seller Tool.",
    ...
    "Follow the contextPriority order supplied in ELYON_CONTEXT_JSON. The current request and verified current-turn evidence come first, then working memory, then conversation, durable memory, and only then background operational history.",
    ...
    "Be concise, practical, decisive, and Elyon-specific.",
...
async function runJarvisBrain({
  ...
  for (const attempt of providerAttempts) {
    const result = await routeAI({
      ...
      messages: [
        ...
        { role: "system", content: `ELYON_CONTEXT_JSON: ${JSON.stringify(context).slice(0, 26000)}` },
  ...
  return {
    ...
    brain: {
      available: true,
      provider: success.result.provider || success.attempt.provider,
      model: success.result.model || success.attempt.model || null,
      fallbackUsed: success.index > 0,
      attempts,
    },
    memory: memoryResult,
    context: {
