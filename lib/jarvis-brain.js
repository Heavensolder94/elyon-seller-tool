...
function sanitizeBrainAttempt(attempt, result, protocolError = null) {
  ...
  return {
    ...
    ok: result?.ok === true && !protocolError,
    error: text(protocolError || error.code, 80) || null,
    errorType: protocolError ? "protocol" : (text(error.type, 80) || null),
...
function validateBrainResponse(raw) {
  const source = text(raw, 30000);
  if (!source) return { ok: false, error: "EMPTY_BRAIN_RESPONSE", payload: null };
  const parsed = extractJsonObject(source);
  if (!parsed) return { ok: false, error: "INVALID_BRAIN_JSON", payload: null };
  const payload = extractBrainPayload(source);
  if (!text(payload.answer)) return { ok: false, error: "MISSING_BRAIN_ANSWER", payload: null };
...
function brainSystemPrompt() {
  return [
    ...
    "Live eBay publishing is approval-bound, not generally locked. Draft mode remains the default. A live action may only use the deterministic Seller-Tool Publishing Gate after current readiness, compliance, eBay-setup and safety checks and either explicit per-listing confirmation or a previously explicit Auto-Live enablement in the dedicated publishing control. Never infer publishing approval from a general automation preference, Brain or Memory content, a playbook, or an agent recommendation. Supplier ordering, refunds, automatic customer-message sending and unapproved legal/compliance mutations remain locked or approval-bound by runtime policy.",
    ...
    "Make the answer string easy to read in chat: use short sections, blank lines and bullet/numbered lists when useful. Keep it concise and normally below 700 words unless the user explicitly asks for more detail.",
    ...
    "Return ONLY one complete valid JSON object with this schema. Never return partial JSON, commentary outside the JSON, or a second JSON object:",
...
async function runJarvisBrain({
  ...
  for (const attempt of providerAttempts) {
    ...
    const validation = result?.ok === true
      ? validateBrainResponse(result.content)
      : { ok: false, error: null, payload: null };
    attempts.push(sanitizeBrainAttempt(attempt, result, validation.error));
    if (result?.ok && validation.ok) {
      success = { result, attempt, index: attempts.length - 1, payload: validation.payload };
      break;
    }
  }

  if (!success) {
    const protocolFailure = attempts.some((attempt) => attempt.errorType === "protocol");
    const failureCode = protocolFailure ? "brain_response_invalid" : "brain_provider_unavailable";
    ...
    return {
      ...
      mode: "brain_degraded",
      error: failureCode,
      answer: protocolFailure
        ? "Mein Jarvis-Brain hat von den verfügbaren KI-Providern keine vollständige, gültig strukturierte Antwort erhalten. Ich gebe bewusst kein abgeschnittenes oder beschädigtes Roh-JSON aus. Bitte versuche die Anfrage erneut oder prüfe den Provider-Status im Integration Center."
        : "Mein Jarvis-Brain konnte gerade keinen konfigurierten KI-Provider erreichen. Die Elyon-Systemfunktionen und Spezial-Agenten bleiben verfügbar; die Provider-Kette und ihre Fehlerdetails können im Systemstatus geprüft werden.",
