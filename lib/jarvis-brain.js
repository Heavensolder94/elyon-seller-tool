...
const DEFAULT_BRAIN_ATTEMPT_TIMEOUT_MS = 45000;
...
async function withAttemptTimeout(promise, timeoutMs, model) {
  const duration = Math.max(100, Number(timeoutMs) || DEFAULT_BRAIN_ATTEMPT_TIMEOUT_MS);
  let timer = null;
  const timeout = new Promise((resolve) => {
    timer = setTimeout(() => resolve({
      ...
      error: {
        code: "TIMEOUT",
        message: `Jarvis Brain model attempt exceeded ${duration} ms.`,
        type: "timeout",
      },
    }), duration);
  });
...
async function runJarvisBrain({
  ...
  attemptTimeoutMs = DEFAULT_BRAIN_ATTEMPT_TIMEOUT_MS,
} = {}) {
  ...
  for (const model of models) {
    const result = await withAttemptTimeout(routeAI({
      provider: "openrouter",
      model,
      task: "jarvis_brain",
      allowFallback: false,
  ...
  if (!success) {
    return {
      ...
      answer: "Mein Jarvis-Brain konnte gerade kein KI-Modell rechtzeitig erreichen. Die Elyon-Systemfunktionen und Spezial-Agenten bleiben verfügbar; für eine freie Brain-Antwort muss die OpenRouter-Verbindung geprüft oder erneut versucht werden.",
