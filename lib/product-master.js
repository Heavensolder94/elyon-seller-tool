...
export function normalizeProduct(input = {}) {
  ...
  if (!title || title === "Unbenanntes Produkt") blockers.push("Titel fehlt oder ist unklar.");
  if (!description) blockers.push("Beschreibung fehlt.");
  if (!image && images.length === 0) blockers.push("Bilder fehlen.");
  if (!supplierUrl) blockers.push("Lieferanten-Link fehlt.");
  if (!buyPrice) blockers.push("Einkaufspreis fehlt.");
  if (!salePrice && suggestedSalePrice) warnings.push(`Verkaufspreis fehlt noch. Vorschlag: ${suggestedSalePrice.toFixed(2)} EUR.`);
  if (!salePrice && !suggestedSalePrice) blockers.push("Verkaufspreis fehlt noch.");
  ...
  const reviewPenalty = warnings.length ? Math.min(15, warnings.length * 10) : 0;
  const blockerPenalty = blockers.length ? Math.min(45, blockers.length * 15) : 0;
  const readinessScore = Math.max(0, Math.min(100, baseScore - reviewPenalty - blockerPenalty));
