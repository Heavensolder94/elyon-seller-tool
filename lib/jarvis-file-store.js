...
function normalizeFileRow(row = {}) {
  return {
    ...
    activeVersion: Number.isInteger(Number(row.active_version)) ? Number(row.active_version) : null,
...
async function createJarvisFileVersion({
  ...
  if (expectedActiveVersion !== undefined && expectedActiveVersion !== null) {
    const expected = Number(expectedActiveVersion);
    const actual = file.activeVersion ?? null;
    if (!Number.isInteger(expected) || expected < 0 || (expected || null) !== actual) {
