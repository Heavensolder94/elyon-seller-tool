...
function reconcilePricing(normalized = {}) {
  ...
  const reconciled = staleZeroFees || staleZeroProfit;
  const profit = reconciled ? calculatedProfit : (storedProfit ?? calculatedProfit);
  const calculatedMargin = salePrice > 0 ? (profit / salePrice) * 100 : 0;
  const marginPercent = reconciled ? calculatedMargin : (storedMargin ?? calculatedMargin);
...
export function normalizeProduct(input = {}) {
  ...
  return applyIdentityReadiness(reconcilePricing(repairSupplier(normalized, adapted)));
}
