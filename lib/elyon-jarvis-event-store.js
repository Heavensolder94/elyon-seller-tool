...
const SENSITIVE_KEY = /^(?:buyer.*|customer.*|.*email.*|.*phone.*|shippingaddress.*|shipto.*|recipient.*|contactaddress.*|firstname|lastname|addressline.*)$/i;
...
function scrubPayload(value, depth = 0) {
  ...
  for (const [key, entry] of Object.entries(value).slice(0, 100)) {
    ...
    const normalizedKey = safeKey.replace(/[_\-\s]/g, "");
    if (!safeKey || SENSITIVE_KEY.test(normalizedKey)) continue;
...
async function indexStored(indexKey, id, createdAt, maxItems, options = {}) {
  const score = Date.parse(text(createdAt)) || Date.now();
  await redisCommand(["ZADD", indexKey, score, id], options);
  const countResponse = await redisCommand(["ZCARD", indexKey], options);
  const count = Number(redisResult(countResponse)) || 0;
  if (count > maxItems) await redisCommand(["ZREMRANGEBYRANK", indexKey, 0, count - maxItems - 1], options);
}

async function listStored(indexKey, prefix, options = {}) {
  const limit = safeLimit(options.limit, 20, 100);
  const idsResponse = await redisCommand(["ZREVRANGE", indexKey, 0, limit - 1], options);
...
export async function ingestJarvisEvent(incoming, options = {}) {
  ...
  await indexStored(EVENT_INDEX_KEY, event.eventId, event.createdAt, MAX_EVENTS, options);
  ...
  await indexStored(JOB_INDEX_KEY, job.jobId, job.createdAt, MAX_JOBS, options);
