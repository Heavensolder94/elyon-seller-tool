...
const SECRET_ENV_NAME = "ELYON_SELLER_ACCESS_TOKEN";
...
function rawAccessSecret() {
  return String(process.env[SECRET_ENV_NAME] ?? "");
}

function accessSecret() {
  return normalizeSecret(rawAccessSecret());
}
...
function equalSecret(left, right) {
  return constantTimeEqual(normalizeSecret(left), normalizeSecret(right));
}
...
export function sellerAccessConfiguration() {
  const raw = rawAccessSecret();
  const normalized = normalizeSecret(raw);
  return {
    configured: Boolean(normalized),
    source: SECRET_ENV_NAME,
    environment: text(process.env.VERCEL_ENV) || "local",
    formatAdjusted: Boolean(raw && raw !== normalized),
  };
}

export function sellerAccessConfigured() {
  return sellerAccessConfiguration().configured;
}
...
export function requireCronOrSellerAccess(req, res, options = {}) {
  ...
  const authorization = text(req?.headers?.authorization);
  const bearer = authorization.match(/^Bearer\s+(.+)$/i)?.[1] || "";
  if (cronSecret && constantTimeEqual(bearer, cronSecret)) {
    setSellerSecurityHeaders(res);
    return true;
  }
