...
import { resolveJarvisManagedFile } from "./jarvis-file-resolver.js";
...
async function readRepositoryBrainDocument(pathname) {
...
async function readBrainDocument(pathname, { env = process.env, resolveManagedFile = resolveJarvisManagedFile } = {}) {
  if (!ALLOWED_PATHS.has(pathname)) throw new Error(`brain_file_path_not_allowed:${pathname}`);
  return resolveManagedFile({
    identifier: pathname,
    env,
    fallback: () => readRepositoryBrainDocument(pathname),
  });
}
...
async function loadJarvisBrainFiles({ command = "", env = process.env, resolveManagedFile = resolveJarvisManagedFile } = {}) {
  const warnings = [];
  let manifest;
  try {
  ...
  for (const entry of manifest.files) {
    ...
    try {
      const resolved = await readBrainDocument(entry.path, { env, resolveManagedFile });
      const markdown = resolved?.content || "";
      if (resolved?.warning) warnings.push(`${resolved.warning}:${entry.id}`);
      ...
      const runtimeMeta = {
        runtimeSource: text(resolved?.source, 40) || "repository",
        runtimeVersion: Number(resolved?.version) || null,
      };
      selected.push(compactLoaded(entry, content, chosenPlaybook
        ? { ...runtimeMeta, playbookId: chosenPlaybook.id }
        : runtimeMeta));
...
function coreBrainMetadata(coreBrain = {}) {
  return {
    ...
    fileSources: (Array.isArray(coreBrain?.core) ? coreBrain.core : []).map((item) => ({
      id: item.id,
      source: item.runtimeSource || "repository",
