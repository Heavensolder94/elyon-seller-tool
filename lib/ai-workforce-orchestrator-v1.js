...
const LISTING_CONTEXT_FIELDS = Object.freeze([
  "title",
  "subtitle",
  "shortDescription",
  "longDescription",
...
function pickListingPatch(generatedContent) {
  const generated = plainObject(generatedContent);
  const nested = plainObject(generated.listingDraft || generated.listing || generated.draft);
  const source = Object.keys(nested).length ? nested : generated;
  const patch = {};
  for (const key of LISTING_CONTEXT_FIELDS) {
    if (!(key in source)) continue;
    const value = source[key];
    if (value === undefined || value === null || value === "") continue;
    patch[key] = safeClone(value);
  }
  return patch;
}

function compactAgentResult(task) {
  const result = plainObject(task?.result);
  return {
    status: text(result.status, 100),
    summary: text(result.summary, 3000),
    findings: list(result.findings, 30),
    recommendations: list(result.recommendations, 30),
    warnings: list(result.warnings, 30),
    blockers: list(result.blockers, 30),
    missingFacts: list(result.missingFacts, 30),
    generatedContent: safeClone(plainObject(result.generatedContent)),
  };
}

function propagateTaskResult(workingContext, task) {
  const agentId = text(task?.agentId, 100);
  if (!agentId) return [];
  const updates = [];
  workingContext.agentResults = plainObject(workingContext.agentResults);
  workingContext.agentResults[agentId] = compactAgentResult(task);
  updates.push(`${agentId}:agentResult`);

  if (agentId === "elyon-listing-specialist") {
    const patch = pickListingPatch(task?.result?.generatedContent);
    if (Object.keys(patch).length) {
      workingContext.listingDraft = {
        ...plainObject(workingContext.listingDraft),
        ...patch,
      };
      workingContext.product = {
        ...plainObject(workingContext.product),
        listingDraft: {
          ...plainObject(workingContext.product?.listingDraft),
          ...patch,
        },
      };
      updates.push("listingDraft");
    }
  }
  return updates;
}

function managerSummary(status, childTasks, workflowType, stopReason = "") {
...
async function runManagerOrchestration({
  ...
  const workingContext = safeClone(plainObject(context));
  const sourceId = sourceIdFromContext(workingContext);
  const waves = workflowWaves(normalizedType, workingContext);
  const childTasks = [];
  const events = [];
  const contextUpdates = [];
  const allowedSet = Array.isArray(allowedAgentIds)
    ? new Set(allowedAgentIds.map((agentId) => text(agentId, 100)).filter(Boolean))
    : null;
  ...
  for (let waveIndex = 0; waveIndex < waves.length; waveIndex += 1) {
    ...
    const denied = allowedSet ? wave.filter((agentId) => !allowedSet.has(agentId)) : [];
    if (denied.length) {
      status = "manual_review_required";
      stopReason = `${denied.map((agentId) => AGENT_STRUCTURE[agentId]?.name || agentId).join(", ")} ist pausiert, deaktiviert oder hat sein Tageslimit erreicht. Der Manager delegiert nicht daran vorbei.`;
      events.push({
    ...
    const waveContext = safeClone(workingContext);
    ...
    for (const task of results) {
      ...
      contextUpdates.push(...propagateTaskResult(workingContext, task));
    }
    events.push({
      type: "wave_completed",
      wave: waveIndex + 1,
  ...
  return {
    ...
    workflowType: normalizedType,
    status,
    summary: managerSummary(status, childTasks, normalizedType, stopReason),
    stopReason,
    sourceId,
    goal: text(goal, 4000),
    maxParallelDelegations: MAX_PARALLEL_DELEGATIONS,
    waves: waves.map((wave) => validateWave(wave)),
    allowedAgentIds: allowedSet ? [...allowedSet] : null,
    childTasks,
    ...
    contextUpdates: Array.from(new Set(contextUpdates)),
    finalContext: workingContext,
