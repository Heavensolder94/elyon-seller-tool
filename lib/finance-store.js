...
function upstashConfig() {
  if (process.env.UPSTASH_REDIS_REST_URL && process.env.UPSTASH_REDIS_REST_TOKEN) {
    return { source: "upstash_redis_rest", url: process.env.UPSTASH_REDIS_REST_URL, token: process.env.UPSTASH_REDIS_REST_TOKEN };
  }
  if (process.env.KV_REST_API_URL && process.env.KV_REST_API_TOKEN) {
    return { source: "vercel_kv_rest", url: process.env.KV_REST_API_URL, token: process.env.KV_REST_API_TOKEN };
  }
  if (process.env.EBAY_TOKEN_STORE_URL && process.env.EBAY_TOKEN_STORE_TOKEN) {
    return { source: "ebay_token_store", url: process.env.EBAY_TOKEN_STORE_URL, token: process.env.EBAY_TOKEN_STORE_TOKEN };
  }
...
function key() {
  return text(process.env.ELYON_FINANCE_STORE_KEY) || DEFAULT_KEY;
}

function filePath() {
  return text(process.env.ELYON_FINANCE_STORE_PATH) || DEFAULT_FILE;
}
...
function auditHash(event, previousHash = "") {
  return crypto.createHash("sha256").update(JSON.stringify(stable({ ...event, previousHash }))).digest("hex");
}

function appendAudit(state, event) {
  const previous = list(state.auditLog).at(-1);
  const base = {
    ...createAuditEvent(event.action || "finance_sync", event, event.actor || "seller"),
    ...event,
    previousHash: text(previous?.hash),
  };
...
function mergeById(existing, incoming, options = {}) {
  ...
  for (const entry of list(incoming)) {
    ...
    if (!previous) {
      byId.set(id, { ...entry, createdAt: entry.createdAt || new Date().toISOString(), updatedAt: new Date().toISOString() });
      continue;
    }
    byId.set(id, {
      ...previous,
      ...
      updatedAt: new Date().toISOString(),
      ...(options.preserveOriginal && previous.original ? { original: previous.original } : {}),
    });
  }
  return [...byId.values()];
}
...
export function mergeFinanceState(currentValue = {}, incomingValue = {}, audit = {}) {
  ...
  appendAudit(next, {
    action: audit.action || "finance_state_merged",
    actor: audit.actor || "seller",
    ...
    metadata: {
      inserted: transactionMerge.inserted,
      updated: transactionMerge.updated,
      duplicates: transactionMerge.duplicates,
...
async function upstashGet() {
  ...
  const response = await fetch(`${config.url.replace(/\/$/, "")}/get/${encodeURIComponent(key())}`, {
...
async function upstashSet(payload) {
  const config = upstashConfig();
  if (!config.url || !config.token) throw new Error("Persistenter Upstash-/KV-Speicher für Elyon Finance fehlt.");
  const response = await fetch(`${config.url.replace(/\/$/, "")}/set/${encodeURIComponent(key())}`, {
    method: "POST",
    headers: { Authorization: `Bearer ${config.token}`, "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  });
  const data = await response.json().catch(() => null);
  if (!response.ok) throw new Error(data?.error || data?.message || `Finance Store HTTP ${response.status}`);
...
async function fileSet(payload) {
  await mkdir(dirname(filePath()), { recursive: true });
  await writeFile(filePath(), `${JSON.stringify(payload, null, 2)}\n`, "utf8");
  return { ok: true, mode: "file", path: filePath(), persistent: false };
}
...
export async function writeFinanceState(payload, audit = {}) {
  ...
  if (selected === "unconfigured") {
    return { ok: false, mode: selected, error: "Persistenter Finanzspeicher fehlt. Lokale Browserdaten bleiben nutzbar." };
  }
