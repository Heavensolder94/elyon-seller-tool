...
function activeMarketplaceId(product) {
  const normalized = normalizeProduct(product);
  const raw = deepestRaw(product);
  return String(
    raw.ebayItemId ||
...
export function upsertProductMasterItem(list, incoming) {
  ...
  if (index >= 0) {
    ...
    const currentEbayItemId = activeMarketplaceId(next[index]);
    const incomingEbayItemId = activeMarketplaceId(incoming);
    if (currentEbayItemId && incomingEbayItemId !== currentEbayItemId) {
      return {
        status: "blocked_active_listing",
        product: current,
        items: next,
        activeMarketplaceId: currentEbayItemId,
      };
    }
