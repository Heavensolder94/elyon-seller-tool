(() => {
  ...
  function decorateAutonomy(agentId) {
    ...
    const unavailableCount = registryModels.filter((item) => item?.enabled !== false && text(item.provider).toLowerCase() === "openrouter" && !text(item.modelId || item.runtimeModel || item.providerModel || OPENROUTER_RUNTIME_MODELS[item.id])).length;
