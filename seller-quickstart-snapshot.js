...
export function emptyDashboardSnapshot() {
  return {
    ...
    kpis: [],
    tasks: [],
    pipeline: { products: 0, readyProducts: 0, liveProducts: 0, openOrders: 0, fulfilledOrders: 0 },
...
export function dashboardSnapshotFromDocument(documentRef) {
  ...
  const kpis = [...root.querySelectorAll(".sd-kpi")].map((node) => ({
  ...
  return {
    ...
    tasks,
    pipeline: {
      products: stepValue("product master"),
      readyProducts: stepValue("listingbereit"),
      liveProducts: stepValue("live"),
