...
function installStyles() {
  ...
  style.textContent = `
    ...
    .sd-list{display:grid;gap:9px}.sd-task,.sd-row{padding:12px 13px;border-radius:15px;background:…377 tokens truncated…d{color:#fca5a5}.sd-note{font-size:10px;color:#64748b;margin-top:10px;text-align:right}
...
function productMasterStatus(payload, error = "") {
  if (error) return "Fehler";
  if (payload && (payload.storage?.configured === true || Array.isArray(payload.products))) return "Verbunden";
  return "Unbekannt";
}

function dashboardMarkup(metrics, tasks) {
  const storageReady = runtime.productPayload?.storage?.configured === true || Array.isArray(runtime.productPayload?.products);
  const productState = productMasterStatus(runtime.productPayload, runtime.errors.products);
