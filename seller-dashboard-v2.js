...
export function normalizeSellerProduct(product = {}) {
  ...
  return {
    ...
    isListed: itemIds.length > 0 || ["live", "active", "published", "manually_listed", "listed"].includes(status),
    isDraft: itemIds.length === 0 && ["draft", "entwurf", "ready_for_manual_listing", "not_listed"].includes(status),
...
function buildTopProducts(orders, products) {
  ...
  orders.filter((order) => !order.isCancelled).forEach((order) => {
    order.lineItems.forEach((item) => {
      ...
      // lineItemCost already contains all purchased units; do not multiply it again.
...
export function buildSellerTasks(metrics, errors = {}) {
  ...
  if (metrics.draftProducts.length) add(55, `${metrics.draftProducts.length} Listing-Entwurf${metrics.draftProducts.length === 1 ? "" : "e"}`, "Noch ohne dokumentierte eBay-Artikelnummer.", "ebayListingTab", "info");
...
function installStyles() {
  ...
  style.textContent = `
    ...
    .sd-pipeline{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:8px}.sd-step{padding:13px 10px;border-radius:16px;background:rgba(2,6,23,.38);border:1px solid rgba(148,163,184,.1);text-align:center}.sd-step strong{display:block;font-size:24px}.sd-step span{display:block;margin-top:5px;color:#94a3b8;font-size:10px}.sd-status{display:flex;justify-content:space-between;gap:12px;align-items:center;padding:11px 12px;border-radius:14px;background:rgba(2,6,23,.38);border:1px solid rgba(148,163,184,.1)}.sd-status span{font-size:11px;color:#cbd5e1}.sd-status strong{font-size:12px;text-align:right}.sd-good{color:#86efac}.sd-warn{color:#fde68a}.sd-bad{color:#fca5a5}.sd-note{font-size:10px;color:#64748b;margin-top:10px;text-align:right}
    @media(max-width:1180px){.sd-kpis{grid-template-columns:repeat(3,minmax(0,1fr))}.sd-main{grid-template-columns:1fr}.sd-two{grid-template-columns:1fr}}@media(max-width:720px){.sd-kpis{grid-template-columns:repeat(2,minmax(0,1fr))}.sd-pipeline{grid-template-columns:repeat(2,minmax(0,1fr))}.sd-actions{width:100%}.sd-range{width:100%;display:grid;grid-template-columns:repeat(3,1fr)}.sd-task{grid-template-columns:10px minmax(0,1fr)}.sd-task button{grid-column:2;justify-self:start}}@media(max-width:430px){.sd-kpis{grid-template-columns:1fr}.sd-pipeline{grid-template-columns:1fr}}
  `;
