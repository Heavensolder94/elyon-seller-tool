    ...
    .map((entry) => {
      const ancestors = Array.isArray(entry?.categoryTreeNodeAncestors)
        ? entry.categoryTreeNodeAncestors
          .slice()
          .reverse()
          .map((ancestor) => text(ancestor?.categoryName, 300))
          .filter(Boolean)
        : [];
...
export default async function handler(req, res) {
  ...
  if (String(req?.method || "GET").toUpperCase() !== "POST") {
    return res.status(405).json({ ok: false, error: "method_not_allowed", message: "Diese Route unterstützt nur POST." });
  }
  ...
  try {
    ...
    return res.status(400).json({ ok: false, error: "unknown_action", message: `Unbekannte Kategorie-Aktion: ${action}` });
  } catch (error) {
