const SHEET_CONFIG = {
  ...
  suppliers: {
    sheetName: "SupplierListe",
    legacySheetNames: ["Supplier Liste"],
    keyColumn: "Supplier"
  },
  sales: {
    sheetName: "Sales & Klarna",
    legacySheetNames: ["Sales", "InventarTracker", "Bestellungen"],
    keyColumn: "Datum"
  },
  costs: {
...
function parsePayload_(e) {
  if (!e || !e.postData || !e.postData.contents) throw new Error("Fehlender JSON-Body.");
  try {
...
function validatePayload_(payload) {
  ...
  if (!expectedToken) throw new Error("SYNC_TOKEN fehlt in den Script Properties.");
  if (!payload || typeof payload !== "object") throw new Error("Payload fehlt.");
  if (payload.token !== expectedToken) throw new Error("Ungultiger Sync Token.");
  if (payload.action !== "upsertRecords") throw new Error("Unbekannte action: " + String(payload.action || ""));
  if (!SHEET_CONFIG[payload.type]) throw new Error("Unbekannter type: " + String(payload.type || ""));
}
...
function validateQuery_(e) {
  ...
  const type = String((e && e.parameter && e.parameter.type) || "").trim();
  if (token !== expectedToken) throw new Error("Ungultiger Sync Token.");
  if (action !== "getRecords") throw new Error("Unbekannte action: " + String(action || ""));
  if (!SHEET_CONFIG[type]) throw new Error("Unbekannter type: " + String(type || ""));
}
...
function upsertRecords_(ss, type, records) {
  ...
  if (keyIndex < 0) throw new Error("Key Column '" + config.keyColumn + "' nicht in '" + config.sheetName + "' gefunden.");
  ...
  records.forEach(function (record) {
    ...
    if (key && existingMap.has(key)) {
      ...
      inserted += 1;
      if (key) existingMap.set(key, sheet.getLastRow());
    }
    processed += 1;
  });
  return {
...
function buildExistingRowIndex_(sheet, keyIndex, width) {
  ...
  values.forEach(function (row, index) {
    const key = normalizeKey_(row[keyIndex]);
    if (key && !map.has(key)) map.set(key, index + 2);
  });
  return map;
}

function normalizeRecord_(record, headers) {
  if (Array.isArray(record)) return padRow_(record, headers.length);
  if (!record || typeof record !== "object") return headers.map(function () { return ""; });
...
function lookupObjectValue_(obj, headerName) {
  ...
  for (let i = 0; i < keys.length; i += 1) {
    if (normalizeFieldName_(keys[i]) === target) return obj[keys[i]];
  }
  return "";
}
