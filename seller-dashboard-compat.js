(function installSellerDashboardCompatibility(global) {
  ...
  function financeLabel(option) {
    ...
    setTextIfChanged(option, `${prefix}Finanzen`);
  ...
  function listingLabel(option, label) {
    ...
    const current = String(option.textContent || "");
    const prefix = current.match(/^\s*(\d+\.\s*)/i)?.[1] || "";
    setTextIfChanged(option, `${prefix}${label}`);
  }
  ...
  function hideNode(node) {
    ...
    if (display !== "none") node.style?.setProperty?.("display", "none", "important");
    if (node.dataset && node.dataset.elyonRetiredPreEbay !== "true") node.dataset.elyonRetiredPreEbay = "true";
  }
  ...
  function normalizePostEbayNavigation(documentRef) {
    ...
    if (menu) {
      ...
      menuOptions(menu).forEach((option) => {
        if (RETIRED_PRE_EBAY_TABS.has(String(option?.value || ""))) removeOption(menu, option);
      });
    ...
    if (modules && Array.isArray(modules.active)) {
      const filtered = modules.active.filter((item) => !RETIRED_PRE_EBAY_TABS.has(String(item?.id || "")));
      if (filtered.length !== modules.active.length) modules.active = filtered;
  ...
  function normalizePostEbayDashboard(documentRef) {
    ...
    for (const heading of root.querySelectorAll(".sd-head h3")) {
      ...
      if (title === "Produktleistung" && panel) {
        ...
        if (button) {
          ...
          setTextIfChanged(button, "Aktive Listings öffnen");
        }
      }
      if (title === "Seller-Pipeline" && panel) {
        const copy = panel.querySelector?.(".sd-head p");
        setTextIfChanged(copy, "eBay ist die Quelle für Entwürfe, aktive Listings und den anschließenden Seller-Betrieb.");
        ...
        if (pipeline) {
          ...
          if (pipeline.style?.getPropertyValue?.("grid-template-columns") !== "repeat(4,minmax(0,1fr))") {
      ...
      if (title === "Datenqualität und Blocker" && panel) {
        hideNode(panel);
        if (panel.parentElement?.style?.getPropertyValue?.("grid-template-columns") !== "1fr") {
    ...
    for (const label of root.querySelectorAll(".sd-status span")) {
      ...
      if (value === "Company-OS-Produkte") setTextIfChanged(label, "Product-Master-Anreicherung");
      if (value === "Automatisches Einstellen / Bestellen") setTextIfChanged(label, "Automatische Lieferantenbestellung");
    }
    ...
    if (roleStrong && /Seller Tool =/i.test(String(roleStrong.textContent || ""))) {
      setTextIfChanged(roleStrong, "Seller Tool = Betrieb ab eBay");
      setTextIfChanged(roleParagraph, "Company OS erstellt und übergibt das Listing. Das Seller Tool übernimmt eBay-Entwürfe, aktive Listings, Bestellungen, Versand, Finanzen und Retouren.");
    }
  ...
  function observeNavigation(documentRef) {
    ...
    normalizePostEbayNavigation(doc);
    if (menu.dataset?.[NAV_OBSERVER_MARKER] === "true") return { installed: false, reason: "already_observing" };
  ...
  function observeDashboard(documentRef) {
    ...
    const observer = new Observer(() => {
      observer.disconnect();
      try {
        normalizePostEbayDashboard(doc);
      } finally {
